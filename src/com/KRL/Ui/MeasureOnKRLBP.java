...
import com.KRL.Blood.BloodBluetoothController;
import com.KRL.Blood.BloodData;
import com.KRL.Blood.BloodMessage;
 ...
 * 
 */
public class MeasureOnKRLBP extends BaseActivity {
	...
	private static KrlBpHandler handler = null;

	private static String btName = "KRL_ECG";// 蓝牙名称
	private static String btMac = null;// 蓝牙名称
	private static Context context;
	private static boolean bpontest = false;
	...
	// BloodBluetoothController mBloodController = new
	// BloodBluetoothController();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.measure_on_krlbp);
		findId();
		context = this;
	...
	private void takeMeasureBp() {
		if (BloodBluetoothController.getInstance().isConnected()) {
			if (!bpontest) {
				BloodBluetoothController.getInstance().startOnceTest();
				initDataTextEdit();
			} else {
				BloodBluetoothController.getInstance().stopOnceTest();
			}
			setTestBtnText();
		}
	}

	private void setTestBtnText() {
		if (!bpontest) {
	 ...
	 * 连接到PC300设备
	 */
	private void connectKrlBp() {
		// if (bluetoothService.getState() == BluetoothService.STATE_NONE) {//
		// 空闲状态才连接
		String address = cache.getDeviceAddress(Cache.KRLBP);// 查找上次綁定的蓝牙
		btMac = address;
		...
		if (device != null)
			...
			T.showLong(this, "没有绑定的血压设备，请查找设备");
		// }
	}
	...
	private void connect(BluetoothDevice device) {
		...
		BloodBluetoothController.getInstance().setDevice(device);
		BloodBluetoothController.getInstance().setHandler(handler);
		BloodBluetoothController.getInstance().stopCurrentConnect();
	 ...
	 * @param address
	 */
	private void connectKrlBp(String address) {
		btMac = address;
		BluetoothDevice device = bluetoothService
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case BluetoothListActivity.REQUEST_CONNECT_DEVICE:
			if (resultCode == Activity.RESULT_OK) {
				...
				cache.saveDeviceAddress(Cache.KRLBP, address);// 保存地址,以便下次自带连接
				connectKrlBp(address);
			}
			break;
		}
	}

	private class KrlBpHandler extends Handler {
		@Override
		public void handleMessage(Message msg) {
			...
			case UPLOAD_RESULT:
				...
				break;
			case BloodMessage.MSG_RECEIVE_BLOODDATA: {
				bpontest = true;
				BloodData data = BloodBluetoothController.getInstance()
						.getBloodData();
				if (!data.isRealPressure()) {
					HighBpEditText.setText(String.valueOf(data
							.getBloodData(BloodData.BLOODDATA_SYS)));
					lowBpEditText.setText(String.valueOf(data
							.getBloodData(BloodData.BLOODDATA_DIA)));
					pulseEditText.setText(String.valueOf(data
							.getBloodData(BloodData.BLOODDATA_PLUSE)));
					uploadButton.setEnabled(true);
					uploadButton.setClickable(true);
					bpontest = false;
				}
				...
				break;
			case BloodMessage.MSG_CONNECT_FAILED: {

			}
				break;
			case BloodMessage.MSG_CONNECT_SUCESS: {
				measureBpButton.setVisibility(View.VISIBLE);
				statusView.setText("已经连接到:"
						+ BloodBluetoothController.getInstance()
								.getDeviceName());
			}
				break;
			case BloodMessage.MSG_RECEIVE_BATTERY: {
	...
	protected void onDestroy() {
		...
		super.onDestroy();
		BloodBluetoothController.getInstance().stopOnceTest();
		BloodBluetoothController.getInstance().stopConnectThread();
		BloodBluetoothController.getInstance().removeHandler();

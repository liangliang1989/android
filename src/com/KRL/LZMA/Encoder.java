...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init()
			{
				com.KRL.LZMA.RangeCoderEncoder.InitBitModels(m_Encoders);
			}

			public void Encode(com.KRL.LZMA.RangeCoderEncoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(com.KRL.LZMA.RangeCoderEncoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
				{
					for (; i >= 0; i--)
					{
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += com.KRL.LZMA.RangeCoderEncoder.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = (symbol >> i) & 1;
					price += com.KRL.LZMA.RangeCoderEncoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder
	{
		...
		{
			com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_choice);
		...
		public void Encode(com.KRL.LZMA.RangeCoderEncoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			int a0 = com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_choice[0]);
			int a1 = com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_choice[0]);
			int b0 = a1 + com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_choice[1]);
			int b1 = a1 + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	com.KRL.LZMA.BinTree			_matchFinder			= null;
	com.KRL.LZMA.RangeCoderEncoder	_rangeEncoder			= new com.KRL.LZMA.RangeCoderEncoder();
	...
	void Create()
	{
		...
		{
			com.KRL.LZMA.BinTree bt = new com.KRL.LZMA.BinTree();
			int numHashBytes = 4;
	...
	void Init()
	{
		BaseInit();
		_rangeEncoder.Init();
		com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_isMatch);
		com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_isRep0Long);
		com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_isRep);
		com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_isRepG0);
		com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_isRepG1);
		com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_isRepG2);
		com.KRL.LZMA.RangeCoderEncoder.InitBitModels(_posEncoders);
	...
	{
		return com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isRepG0[state]) + com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isRepG0[state]);
			price += com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		}
		else
		{
			price = com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isRepG1[state]);
			else
			{
				price += com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRepG1[state]);
				price += com.KRL.LZMA.RangeCoderEncoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) + _literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
		_optimum[1].MakeAsChar();
		int matchPrice = com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isRep[_state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) + _literalEncoder.GetSubCoder(position, _matchFinder.GetIndexByte(0 - 2)).GetPrice(!Base.StateIsCharState(state), matchByte, currentByte);
			...
			matchPrice = curPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRep[state2]);
					{
			...
			for (int repIndex = 0; repIndex < Base.kNumRepDistances; repIndex++)
			{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice = repMatchPrice + GetRepPrice(repIndex, lenTest, state, posState) + com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) + _literalEncoder.GetSubCoder(position + lenTest, _matchFinder.GetIndexByte(lenTest - 1 - 1)).GetPrice(true, _matchFinder.GetIndexByte(lenTest - 1 - (reps[repIndex] + 1)), _matchFinder.GetIndexByte(lenTest - 1));
						state2 = Base.StateUpdateChar(state2);
						posStateNext = (position + lenTest + 1) & _posStateMask;
						int nextMatchPrice = curAndLenCharPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isRep[state]);
					...
					if (lenTest == _matchDistances[offs])
					{
						if (lenTest < numAvailableBytesFull)
						{
							...
							if (lenTest2 >= 2)
							{
								...
								int posStateNext = (position + lenTest) & _posStateMask;
								int curAndLenCharPrice = curAndLenPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) + _literalEncoder.GetSubCoder(position + lenTest, _matchFinder.GetIndexByte(lenTest - 1 - 1)).GetPrice(true, _matchFinder.GetIndexByte(lenTest - (curBack + 1) - 1), _matchFinder.GetIndexByte(lenTest - 1));
								state2 = Base.StateUpdateChar(state2);
								posStateNext = (position + lenTest + 1) & _posStateMask;
								int nextMatchPrice = curAndLenCharPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
								int nextRepMatchPrice = nextMatchPrice + com.KRL.LZMA.RangeCoderEncoder.GetPrice1(_isRep[state2]);
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << com.KRL.LZMA.RangeCoderEncoder.kNumBitPriceShiftBits);

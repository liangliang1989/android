...
public class GlycuresisTureList extends Activity{
	...
	private void initView() {
		...
		tittle_tv=(TextView) findViewById(R.id.hypertension_tittle);
		tittle_tv.setText("2型糖尿病患者随访服务记录表");
		datas = getDatas();
	...
	private void jump(String sysId) {
		...
		intent.putExtra(GlycuresisTure.SYS_ID, sysId);
		// intent.putExtra(HANDLER, handler);

		startActivityForResult(intent, GlycuresisTure.REQUEST_FRESH);
	}

	private List<String[]> getFromDb() {
		List<String[]> content = new ArrayList<String[]>();
		Cursor cursor = dbService.query(GlycuresisTable.GLYCURESIS_TABLE, GlycuresisTable.SERIAL_ID,
				Tables.getSerialId());
		L.i("getFromDb cursor.getCount()", cursor.getCount() + "");
		while (cursor.moveToNext()) {
			String[] line = new String[3];
			line[0] = getCursorString(cursor, DataOpenHelper.SYS_ID);
			line[1] = getCursorString(cursor, GlycuresisTable.REPORT_DATE);
			line[2] = getCursorString(cursor, GlycuresisTable.TUREDOCTOR);
			content.add(line);
		}
		cursor.close();
		return content;
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
		case GlycuresisTure.REQUEST_FRESH:
			addContent();
			break;
		}
	}

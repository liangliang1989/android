...
public class MeasureOnPC300 extends BaseActivity {
	...
	protected void upload() throws JSONException {
		...
		if (dbp.length() > 0) {// 舒张压有数据，可以上传血压值
			...
			cv.put(Tables.TABLE_NAME, Tables.TABLE_BP);
			cv.put(Tables.SBP, sbp);
			cv.put(Tables.DBP, dbp);
			cv.put(Tables.PULSE, pulse);
			JSONObject data = getDefaltAttrs();
			data.put(Tables.SBP, sbp);
			data.put(Tables.DBP, dbp);
			data.put(Tables.PULSE, pulse);
			uploadInBack(data, WebService.PATH_BP, cv);

		}
		String temp = tempEditText.getText().toString();
		if (temp.length() > 0) {// 体温有数据
			ContentValues cv = getDefaultValues();
			cv.put(Tables.TEMP, temp);
			cv.put(Tables.TABLE_NAME, Tables.TABLE_TEMP);
			JSONObject data = getDefaltAttrs();
			data.put(Tables.TEMP, temp);
			uploadInBack(data, WebService.PATH_TEMP, cv);
		}
		String bo = boEditText.getText().toString();
		if (bo.length() > 0) {// 血氧有数据
			ContentValues cv = getDefaultValues();
			cv.put(Tables.BO, bo);
			cv.put(Tables.TABLE_NAME, Tables.TABLE_BO);
			JSONObject data = getDefaltAttrs();
			data.put(Tables.BO, bo);
			uploadInBack(data, WebService.PATH_BO, cv);
		}
	}
	...
	protected void uploadEcg() throws Exception {
		...
		ContentValues cv = getDefaultValues();
		cv.put(Tables.ECG, array.toString());
		cv.put(Tables.TABLE_NAME, Tables.ECG);
		uploadInBack(data, WebService.PATH_ECG, cv);
	}
